package com.zyp.plugin.core.contentprovider
...
class StubContentProvider : ContentProvider() {
    ...
    override fun onCreate(): Boolean {
        uriMatcher?.addURI("com.zyp.stubprovider", "plugin1", 0)
        uriMatcher?.addURI("com.zyp.stubprovider", "plugin2", 0)
        return true
    }
    ...
    private fun loadPluginProvider() {
        if (pluginProvider == null) {
            pluginProvider = pluginClassLoader?.loadClass("com.zyp.plugin.PluginContentProvider")?.newInstance() as ContentProvider?
        }
    }
